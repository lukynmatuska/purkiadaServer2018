...
while True:
    try:
        ...
        tmp = users_file.readlines()
        user_names = []
        user_passwords = []
        for str in tmp:
            user_names.append(tmp.splt("-")[0])
            user_passwords.append(tmp.splt("-")[1])
        users_file.close()
        ...
        users_file.write("uzivatel-uzivatel\n")
...
def one_user(c, a):
    try:
        ...
        while not user.connected:
            ...
            for username in user_names:#loadTable.users:
                tmpPswd = user_names.index(username)#loadTable.users.index(username)
                ...
                if user.name == username and user_passwords[tmpPswd] == user.pswd:
                    #print("True")
                    c.send("True".encode())
                    user.connected = True
                    user.scoreAdd(1)
